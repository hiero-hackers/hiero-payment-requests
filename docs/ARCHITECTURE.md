...
| `match/`                    | The matching rule as a three-stage pipeline: correlate (strategy, injectable — memo default, `byUniqueAmount` shipped) → tally credits → classify. Canonicalization (dedup + consensus order) is the pipeline's, so no strategy can lose it. `amounts.ts` assigns distinct amounts for the memo-free strategy. |
