...
describe("the facts a merchant needs to tell apart", () => {
  it("reports a shortfall rather than silently failing", () => {
    const result = match(REQUEST, [payment({ credits: [{ account: "0.0.1234", asset: USDC, amount: 60_000000n }] })]);
    expect(result).toMatchObject({ status: "underpaid", received: 60_000000n, shortfall: 40_000000n });
  });

  it("aggregates several payments carrying the same reference", () => {
    const half = (id: string) => payment({ transactionId: id, credits: [{ account: "0.0.1234", asset: USDC, amount: 50_000000n }] });
  ...
  it("surfaces a DOUBLE PAYMENT as overpaid with both transactions — never swallowed", () => {
    const result = match(REQUEST, [payment({ transactionId: "a" }), payment({ transactionId: "b" })]);
    expect(result).toMatchObject({ status: "overpaid", received: 200_000000n, excess: 100_000000n });
  ...
  it("distinguishes the wrong asset from unpaid", () => {
    ...
    const result = match(REQUEST, [payment({ credits: [{ account: "0.0.1234", asset: hbar, amount: 100_000000n }] })]);
...
describe("the traps", () => {
  it("reads the recipient's CREDIT, so an HTS custom fee shows as a shortfall", () => {
    ...
    const result = match(REQUEST, [payment({ credits: [{ account: "0.0.1234", asset: USDC, amount: 98_000000n }] })]);
    expect(result).toMatchObject({ status: "underpaid", received: 98_000000n, shortfall: 2_000000n });
  });

  it("ignores a payment that credited somebody else", () => {
    expect(match(REQUEST, [payment({ credits: [{ account: "0.0.9999", asset: USDC, amount: 100_000000n }] })]).status).toBe("wrong-asset");
  });
